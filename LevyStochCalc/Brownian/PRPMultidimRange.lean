...
variable (W : MultidimBrownianMotion P d) {T : ℝ}

/-- The `L²` ranges of the Itô integrals against distinct coordinates are orthogonal. -/
theorem orthogonalFamily_itoRange (hT : 0 < T) :
    OrthogonalFamily ℝ
      (fun i : Fin d => (itoRange (W.W i) (W.isBrownianFiltration_natural i) hT :
        Submodule ℝ (Lp ℝ 2 P)))
      (fun i => (itoRange (W.W i) (W.isBrownianFiltration_natural i) hT).subtypeₗᵢ) := by
  haveI : Fact ((1 : ℝ≥0∞) ≤ 2) := ⟨by norm_num⟩
  intro i j hij u v
  obtain ⟨G, hG⟩ := u.2
  obtain ⟨K, hK⟩ := v.2
  have hcross : ∫ ω, G.integral (W.W i) (W.isBrownianFiltration_natural i) ω
      * K.integral (W.W j) (W.isBrownianFiltration_natural j) ω ∂P = 0 :=
    integral_stochasticIntegral_mul_eq_zero W hij G.measurable_uncurry G.progressive
...
theorem isClosed_iSup_itoRange (hT : 0 < T) :
    IsClosed ((⨆ i : Fin d, itoRange (W.W i) (W.isBrownianFiltration_natural i) hT :
      Submodule ℝ (Lp ℝ 2 P)) : Set (Lp ℝ 2 P)) := by
  haveI : Fact ((1 : ℝ≥0∞) ≤ 2) := ⟨by norm_num⟩
  haveI : ∀ i : Fin d,
      CompleteSpace (itoRange (W.W i) (W.isBrownianFiltration_natural i) hT) := fun i =>
    (isClosed_itoRange (W.W i) (W.isBrownianFiltration_natural i) hT).completeSpace_coe
  exact isClosed_iSup_of_orthogonalFamily (orthogonalFamily_itoRange W hT)

/-- The multidimensional Itô integral of a family of admissible integrands. -/
noncomputable def vectorIntegral
    (G : ∀ _ : Fin d, HorizonIntegrand P W.naturalFiltration T) : Ω → ℝ :=
  fun ω => ∑ i : Fin d, (G i).integral (W.W i) (W.isBrownianFiltration_natural i) ω

theorem memLp_vectorIntegral (G : ∀ _ : Fin d, HorizonIntegrand P W.naturalFiltration T) :
    MemLp (vectorIntegral W G) 2 P :=
  memLp_finsetSum (f := fun (i : Fin d) ω =>
    (G i).integral (W.W i) (W.isBrownianFiltration_natural i) ω) Finset.univ
    fun i _ => HorizonIntegrand.memLp (W.W i) (W.isBrownianFiltration_natural i) (G i)

theorem integral_vectorIntegral_eq_zero (hT : 0 ≤ T)
    (G : ∀ _ : Fin d, HorizonIntegrand P W.naturalFiltration T) :
    ∫ ω, vectorIntegral W G ω ∂P = 0 := by
  have hrw : vectorIntegral W G = fun ω => ∑ i : Fin d,
      (G i).integral (W.W i) (W.isBrownianFiltration_natural i) ω := rfl
  rw [hrw, integral_finsetSum _ fun i _ =>
    (HorizonIntegrand.memLp (W.W i) (W.isBrownianFiltration_natural i) (G i)).integrable
      (by norm_num)]
  exact Finset.sum_eq_zero fun i _ =>
    HorizonIntegrand.integral_mean_zero (W.W i) (W.isBrownianFiltration_natural i) hT (G i)

theorem aestronglyMeasurable_vectorIntegral
    (G : ∀ _ : Fin d, HorizonIntegrand P W.naturalFiltration T) :
    AEStronglyMeasurable[W.naturalFiltration T] (vectorIntegral W G) P := by
  set F : Fin d → Ω → ℝ :=
    fun i => (G i).integral (W.W i) (W.isBrownianFiltration_natural i) with hF
  have hcomp : ∀ i : Fin d, AEStronglyMeasurable[W.naturalFiltration T] (F i) P := fun i =>
    HorizonIntegrand.aestronglyMeasurable (W.W i) (W.isBrownianFiltration_natural i) (G i)
  have key : ∀ s : Finset (Fin d),
      AEStronglyMeasurable[W.naturalFiltration T] (fun ω => ∑ i ∈ s, F i ω) P := by
    intro s
  ...
  have hgoal : vectorIntegral W G = fun ω => ∑ i ∈ (Finset.univ : Finset (Fin d)), F i ω := rfl
  rw [hgoal]
...
theorem exists_vectorIntegral_of_mean_zero (hT : 0 < T)
    (hsep : ∀ r : Ω → ℝ, MemLp r 2 P → AEStronglyMeasurable[W.naturalFiltration T] r P →
      (∫ ω, r ω ∂P = 0) →
      (∀ (i : Fin d) (G : HorizonIntegrand P W.naturalFiltration T),
        ∫ ω, r ω * G.integral (W.W i) (W.isBrownianFiltration_natural i) ω ∂P = 0) →
      r =ᵐ[P] 0)
    {Z : Ω → ℝ} (hZ2 : MemLp Z 2 P)
    (hZm : AEStronglyMeasurable[W.naturalFiltration T] Z P) (hZ0 : ∫ ω, Z ω ∂P = 0) :
    ∃ G : ∀ _ : Fin d, HorizonIntegrand P W.naturalFiltration T, Z =ᵐ[P] vectorIntegral W G := by
  haveI : Fact ((1 : ℝ≥0∞) ≤ 2) := ⟨by norm_num⟩
  set V : Fin d → Submodule ℝ (Lp ℝ 2 P) := fun i =>
    itoRange (W.W i) (W.isBrownianFiltration_natural i) hT with hVdef
  haveI hcl : IsClosed ((⨆ i, V i : Submodule ℝ (Lp ℝ 2 P)) : Set (Lp ℝ 2 P)) :=
    isClosed_iSup_itoRange W hT
  ...
  have hproj : ∑ i, (V i).starProjection y = y :=
    (orthogonalFamily_itoRange W hT).sum_projection_of_mem_iSup y hy
  have hcomp : ∀ i : Fin d, ∃ G : HorizonIntegrand P W.naturalFiltration T,
      (((V i).starProjection y : Lp ℝ 2 P) : Ω → ℝ)
        =ᵐ[P] G.integral (W.W i) (W.isBrownianFiltration_natural i) :=
    fun i => (V i).starProjection_apply_mem y
  choose G hG using hcomp
  have hyfun : (y : Ω → ℝ) =ᵐ[P] vectorIntegral W G := by
  ...
  have hsplit : Z =ᵐ[P] fun ω => vectorIntegral W G ω + (z : Ω → ℝ) ω := by
  ...
  have hzmeas : AEStronglyMeasurable[W.naturalFiltration T] (z : Ω → ℝ) P := by
    have hd : (z : Ω → ℝ) =ᵐ[P] fun ω => Z ω - vectorIntegral W G ω := by
      filter_upwards [hsplit] with ω e1
      rw [e1]; ring
    exact (hZm.sub (aestronglyMeasurable_vectorIntegral W G)).congr hd.symm
  have hyint : Integrable (vectorIntegral W G) P :=
    (memLp_vectorIntegral W G).integrable (by norm_num)
  ...
  have hzmean : ∫ ω, (z : Ω → ℝ) ω ∂P = 0 := by
    ...
    rw [integral_add hyint hzint, integral_vectorIntegral_eq_zero W hT.le G, zero_add] at hone
    rw [← hone, hZ0]
  have hzperp : ∀ (i : Fin d) (K : HorizonIntegrand P W.naturalFiltration T),
      ∫ ω, (z : Ω → ℝ) ω * K.integral (W.W i) (W.isBrownianFiltration_natural i) ω ∂P = 0 := by
    intro i K
    have hmem : (HorizonIntegrand.memLp (W.W i) (W.isBrownianFiltration_natural i) K).toLp
        (K.integral (W.W i) (W.isBrownianFiltration_natural i))
    ...
    filter_upwards [MemLp.coeFn_toLp
      (HorizonIntegrand.memLp (W.W i) (W.isBrownianFiltration_natural i) K)] with ω e1
