...
theorem integral_mul_cross_increment_eq_zero {i j : Fin d} (hij : i ≠ j) {a b : ℝ} (ha : 0 < a)
  ...
  have ht : (0 : ℝ) ≤ t := le_trans (le_trans ha.le hab.le) hbt
  have hone : ∀ _ω : Ω, |(1 : ℝ)| ≤ 1 := fun _ => le_of_eq abs_one
  have h1 := mul_sub_stochasticIntegralBrownian_ae (W.W i) (hcoord i) H hHm hHp hHs ha hab
    hZm hZb hZa hbt
  have h2 := mul_sub_stochasticIntegralBrownian_ae (W.W j) (hcoord j) K hKm hKp hKs ha hab
    (Z := fun _ => (1 : ℝ)) measurable_const hone stronglyMeasurable_const hbt
  have hcross := MultidimBrownianMotion.integral_stochasticIntegral_mul_eq_zero W hij hcoord 𝒲
    ((twoCell ha hab Z hZm hZb).measurable_uncurry_eval_mul hHm)
    ((twoCell ha hab Z hZm hZb).progressivelyMeasurable_eval_mul ℱ
      (twoCell_adapted ha hab hZm hZb hZa) hHp)
    ((twoCell ha hab Z hZm hZb).lintegral_eval_mul_sq_lt_top hHs)
    ((twoCell ha hab (fun _ => (1 : ℝ)) measurable_const hone).measurable_uncurry_eval_mul hKm)
    ((twoCell ha hab (fun _ => (1 : ℝ)) measurable_const hone).progressivelyMeasurable_eval_mul ℱ
      (twoCell_adapted ha hab measurable_const hone stronglyMeasurable_const) hKp)
    ((twoCell ha hab (fun _ => (1 : ℝ)) measurable_const hone).lintegral_eval_mul_sq_lt_top hKs)
    ht
...
theorem condExp_mul_cross_increment_eq_zero {i j : Fin d} (hij : i ≠ j) {a b : ℝ} (ha : 0 < a)
  ...
  have hkey := integral_mul_cross_increment_eq_zero W hcoord 𝒲 hHm hHp hHs hKm hKp hKs hij
    ...
    ((measurable_const : Measurable fun _ : Ω => (1 : ℝ)).indicator (ℱ.le a s hs)) hbd hind hbt
