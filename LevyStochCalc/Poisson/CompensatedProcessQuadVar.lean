...
noncomputable def stageEval (n : ℕ) (p : Ω × ℝ × E) : ℝ :=
  (master N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n).2.eval p.2.1 p.2.2 p.1

lemma measurable_stageEval (n : ℕ) :
    Measurable (stageEval N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n) :=
  (master N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n).2.eval_measurable

/-- The stage integrands converge to `φ` in `L²` of the horizon measure at every time. -/
lemma stageEval_tendsto (t : ℝ) :
    Tendsto (fun n => eLpNorm (stageEval N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n
      - fun p : Ω × ℝ × E => φ p.1 p.2.1 p.2.2) 2 (horizonMeasure ν P t)) atTop (𝓝 0) := by
  have hsq : Tendsto (fun n => eLpNorm (stageEval N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n
    ...
    obtain ⟨N₀, hN₀⟩ : ∃ N₀ : ℕ, t ≤ stageHorizon N₀ := by
      ...
      exact ⟨k, hk.le⟩
    have hbound : ∀ n, N₀ ≤ n → eLpNorm (stageEval N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n
        - fun p : Ω × ℝ × E => φ p.1 p.2.1 p.2.2) 2 (horizonMeasure ν P t) ^ (2 : ℝ)
      ...
      calc eLpNorm (stageEval N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n
            - fun p : Ω × ℝ × E => φ p.1 p.2.1 p.2.2) 2 (horizonMeasure ν P t) ^ (2 : ℝ)
          = ∫⁻ p, (‖(stageEval N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n
...
lemma memLp_stageEval (n : ℕ) {t : ℝ} (ht : 0 ≤ t) :
    MemLp (stageEval N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n) 2 (horizonMeasure ν P t) := by
  refine ⟨(measurable_stageEval N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n).aestronglyMeasurable,
    ?_⟩
  refine (ENNReal.rpow_lt_top_iff_of_pos (by norm_num : (0 : ℝ) < 2)).1 ?_
  rw [LevyStochCalc.Brownian.Ito.eLpNorm_sq_eq_lintegral_nnnorm_sq]
  unfold stageEval
  ...
  have h1 := G.lintegral_integral_sq_at N hℱ (master_adapted N ℱ hℱ φ h_meas h_progMeas
    h_sq_int_global n)
    ht
...
lemma stage_compensator_tendsto_L1 (t : ℝ) :
    Tendsto (fun n => eLpNorm (fun ω =>
      (master N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n).2.compensator t ω
        - compensator ν φ t ω) 1 P) atTop (𝓝 0) := by
  rcases le_or_gt t 0 with ht | ht
  · refine Tendsto.congr' ?_ tendsto_const_nhds
    filter_upwards with n
    symm
    rw [show (fun ω => (master N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n).2.compensator t ω
  ...
  · have hgmem := memLp_phi_horizon φ h_meas h_sq_int_global ht
    have hjoint : Tendsto (fun n => eLpNorm (fun p : Ω × ℝ × E =>
        (stageEval N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n p) ^ 2 - (φ p.1 p.2.1 p.2.2) ^ 2) 1
        (horizonMeasure ν P t)) atTop (𝓝 0) :=
      LevyStochCalc.Brownian.Ito.tendsto_eLpNorm_one_sq_sub
        (fun n => (measurable_stageEval N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n).aemeasurable)
        h_meas.aemeasurable hgmem.2.ne (stageEval_tendsto N ℱ hℱ φ h_meas h_progMeas
          h_sq_int_global t)
    have hmarg : ∀ n, eLpNorm (fun ω =>
        (master N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n).2.compensator t ω
          - compensator ν φ t ω) 1 P
        ≤ eLpNorm (fun p : Ω × ℝ × E =>
          (stageEval N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n p) ^ 2 - (φ p.1 p.2.1 p.2.2) ^
            2) 1
          (horizonMeasure ν P t) := by
      intro n
      have hGsq : Integrable (fun p : Ω × ℝ × E =>
          (stageEval N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n p) ^ 2) (horizonMeasure ν P t) :=
      ...
      have hd : Integrable (fun p : Ω × ℝ × E =>
          (stageEval N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n p) ^ 2 - (φ p.1 p.2.1 p.2.2) ^ 2)
      ...
      have hcomb : (master N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n).2.compensator t ω
          - compensator ν φ t ω
          = ∫ y, ((stageEval N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n (ω, y)) ^ 2
...
theorem martingale_quadVar_process :
  ...
  refine LevyStochCalc.Brownian.Ito.martingale_of_tendsto_eLpNorm_one
    (M := fun n t ω => (stageIntegral N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n t ω) ^ 2
      - (master N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n).2.compensator t ω)
    (fun n => (master N ℱ hℱ φ h_meas h_progMeas h_sq_int_global
      n).2.martingale_sq_sub_compensator N hℱ
      (master_adapted N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n))
    (fun n t => ((master N ℱ hℱ φ h_meas h_progMeas h_sq_int_global
      n).2.martingale_sq_sub_compensator N hℱ (master_adapted N ℱ hℱ φ h_meas h_progMeas
  ...
  have hX : Tendsto (fun n => eLpNorm (fun ω =>
      (stageIntegral N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n t ω) ^ 2
  ...
  have hXaesm : AEStronglyMeasurable (fun ω =>
    ...
    (memLp_stageIntegral N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n
  ...
  have hYaesm : AEStronglyMeasurable (fun ω =>
    ...
    ((master N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n).2.integrable_compensator
      t).aestronglyMeasurable.sub
      (integrable_compensator φ h_meas h_sq_int_global t).aestronglyMeasurable
  calc eLpNorm ((fun ω => (stageIntegral N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n t ω) ^ 2
          - (master N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n).2.compensator t ω)
        - fun ω => (process N ℱ hℱ φ h_meas h_progMeas h_sq_int_global t ω) ^ 2
          - compensator ν φ t ω) 1 P
      = eLpNorm ((fun ω => (stageIntegral N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n t ω) ^ 2
            - (process N ℱ hℱ φ h_meas h_progMeas h_sq_int_global t ω) ^ 2)
          - fun ω => (master N ℱ hℱ φ h_meas h_progMeas h_sq_int_global n).2.compensator t ω
