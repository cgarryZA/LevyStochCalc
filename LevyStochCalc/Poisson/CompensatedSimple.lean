...
For a Poisson random measure `N` on `[0,T] × E` with intensity `dt ⊗ ν`, the
*compensated* measure is `Ñ(dt, de) := N(dt, de) − ν(de) dt`. This file builds
the integral against `Ñ` for **simple predictable** integrands and proves the
L²-isometry on that class (Applebaum 2009 §4.2, Stages 1–2):

* `SimplePredictable` — integrands `φ = ∑_i ξ_i · 1_{(t_i, t_{i+1}] × A_i}` with
  `ξ_i` `ℱ_{t_i}`-bounded and `ν(A_i) < ∞`.
* `simpleIntegral` — the finite sum `∑_i ξ_i Ñ((t_i, t_{i+1}] × A_i)`, with its
  mean-zero / second-moment / diagonal / off-diagonal identities, culminating in
  the simple-integrand L²-isometry `simpleIntegral_isometry` and L²-membership.

The L²-completion to the full integral (`stochasticIntegral`) and its
martingale / isometry / càdlàg properties are in `Poisson/Compensated.lean`.
-/
