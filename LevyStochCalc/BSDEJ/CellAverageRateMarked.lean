...
`K * δ * |φ e|`, so the marked energy of `U - cellTimeAverage_U π U` over the horizon
...
theorem cellTimeAverage_U_sub_le_of_lipschitz (hmono : StrictMono π) (h0 : π 0 = 0)
    (hT : π (Fin.last M) = T) (hδ : ∀ n : Fin M, π n.succ - π n.castSucc ≤ δ)
    (hlip : ∀ s t : ℝ, |h s - h t| ≤ K * |s - t|) {s : ℝ} (hs : s ∈ Set.Ioc (0 : ℝ) T)
    (ω : Ω) (e : E) :
    |h s * φ e - cellTimeAverage_U π (fun u (_ : Ω) (e : E) => h u * φ e) s ω e|
      ≤ K * δ * |φ e| := by
  classical
  obtain ⟨n₀, hn₀, huniq⟩ := existsUnique_mem_cell hmono h0 hT hs
  have hsum : cellTimeAverage_U π (fun u (_ : Ω) (e : E) => h u * φ e) s ω e
...
theorem lintegral_sq_sub_cellTimeAverage_U_le_of_lipschitz (ν : Measure E)
    ...
    ∫⁻ s in Set.Icc (0 : ℝ) T, ∫⁻ e, (‖h s * φ e
        - cellTimeAverage_U π (fun u (_ : Ω) (e : E) => h u * φ e) s ω e‖₊ : ℝ≥0∞) ^ 2 ∂ν
  ...
  have hinner : ∀ {s : ℝ}, s ∈ Set.Ioc (0 : ℝ) T →
    ...
    have hpt : ∀ e : E, (‖h s * φ e
        - cellTimeAverage_U π (fun u (_ : Ω) (e : E) => h u * φ e) s ω e‖₊ : ℝ≥0∞) ^ 2
        ≤ ENNReal.ofReal ((K * δ) ^ 2) * (‖φ e‖₊ : ℝ≥0∞) ^ 2 := by
      intro e
      have hb := cellTimeAverage_U_sub_le_of_lipschitz (φ := φ) hmono h0 hT hδ hlip hs ω e
      set x := h s * φ e
        - cellTimeAverage_U π (fun u (_ : Ω) (e : E) => h u * φ e) s ω e with hxdef
      clear_value x
...
`[0, T]`. -/
theorem markedEnergy_sub_cellTimeAverage_U_le_of_lipschitz (P : Measure Ω)
    ...
    Poisson.Compensated.markedEnergy P ν T
        (fun ω s e => h s * φ e
          - cellTimeAverage_U π (fun u (_ : Ω) (e : E) => h u * φ e) s ω e)
      ≤ ENNReal.ofReal ((K * δ) ^ 2 * T) * ∫⁻ e, (‖φ e‖₊ : ℝ≥0∞) ^ 2 ∂ν := by
  ...
  calc ∫⁻ ω, ∫⁻ s in Set.Icc (0 : ℝ) T, ∫⁻ e, (‖h s * φ e
      ...
      ≤ ∫⁻ _ : Ω, ENNReal.ofReal ((K * δ) ^ 2 * T) * ∫⁻ e, (‖φ e‖₊ : ℝ≥0∞) ^ 2 ∂ν ∂P :=
        lintegral_mono fun ω =>
          lintegral_sq_sub_cellTimeAverage_U_le_of_lipschitz ν hmono h0 hT hδ hlip ω
    _ = ENNReal.ofReal ((K * δ) ^ 2 * T) * ∫⁻ e, (‖φ e‖₊ : ℝ≥0∞) ^ 2 ∂ν := by
