...
/-!
# The Itô–Lévy formula for a jump diffusion and a `C^{1,2}` state function

The Itô–Lévy formula for a state function of class `C^{1,2}` along a jump diffusion, relative to
the filtration of the solution's SDE data, with no bound on the derivatives of the state
function: the only hypotheses beyond the SDE data are the joint measurability of the
coefficients, the window energy of the drift along the path, and the admissibility of the two
derived integrands `(∇u)ᵀσ` and `u(x + γ) − u(x)` together with the integrability of the
compensator drift.

The state function is cut off outside a ball, the bounded-derivative formula for a `C^{1,2}`
state function applies to the cut-off, and every term transfers back on the event that the path
stays in the ball over the window: the continuous side term by term, the jump side as a sum,
through the arrival-time jump relation of the solution. Almost every path stays in some ball.
Since a function that is `C²` jointly in time and state is of class `C^{1,2}`, the formula for a
jointly `C²` state function is a case of the one for a `C^{1,2}` state function.

## Main statements

* `LevyStochCalc.Ito.JumpFormula.itoLevyFormula_jumpResidual_of_sdeData_general_of_leftLim_c12` —
  the residual form along a solution with càdlàg paths at every sample point.
* `LevyStochCalc.Ito.JumpFormula.itoLevyFormula_jumpResidual_of_sdeData_general_c12` — the
  residual form along any jump diffusion, through its càdlàg representative.
* `LevyStochCalc.Ito.JumpFormula.itoLevyFormula_general_c12` — the four-term form for a
  `C^{1,2}` state function.
* `LevyStochCalc.Ito.JumpFormula.itoLevyFormula_general_of_c12` — the four-term form for a
  jointly `C²` state function.
-/
...
theorem itoLevyFormula_jumpResidual_of_sdeData_general_of_leftLim_c12 (x₀ : Fin n → ℝ)
    ...
    ∀ᵐ ω ∂P,
      (u T (X.X T ω) - u 0 (X.X 0 ω)
        ...
        - MultidimBrownianMotion.stochasticIntegral W S.ℱ S.isBrownian
            ...
            (fun j => measurable_diffusionIntegrand_path_c12 hu hσmeas X.measurable_path j)
            (fun j => progressivelyMeasurable_diffusionIntegrand_path_c12 hu hσmeas S.X_prog j)
            h_sigmaGrad_sq T ω)
      = LevyStochCalc.Poisson.Compensated.stochasticIntegral N S.ℱ S.isPoisson
          ...
          (measurable_jumpIncrement_path_c12 hu hγmeas X.measurable_path)
          (markedProgressivelyMeasurable_jumpIncrement_path_c12 hu hγmeas S.X_prog)
          h_jumpInt_sq T ω
  ...
  have hCdu : ∀ᵐ ω ∂P, IntegrableOn (fun q : ℝ × E =>
    ...
    refine integrableOn_window_univ_of_lintegral_lt_top
      ((measurable_compensatorDriftIntegrand_path_c12 hu hγmeas hΛm).comp measurable_prodMk_left) ?_
  ...
  have hmain : ∀ m : ℕ, ∀ᵐ ω ∂P, (0 < m ∧ T < 3 * (m : ℝ)) →
    ...
    have hv : IsC12 (cutoffFun₂ u (2 * (m : ℝ))) :=
      isC12_cutoffFun₂ hu _
    obtain ⟨K, -, hK₀, hK₁, hK₂⟩ := exists_globalBound_cutoffFun₂_c12 hu hR
    -- the bounded-derivative formula for the cut-off
    have hM16 := itoLevyFormula_jumpResidual_of_sdeData_of_leftLim_c12 W N coeffs x₀ X S hℱ0 hnull
    ...
    have hcont := ae_continuousSide_cutoffFun₂_eq W S.ℱ S.isBrownian u hm hTm hT hadapt hXright
      (fun j => measurable_diffusionIntegrand_path_c12 hv hσmeas X.measurable_path j)
      (fun j => progressivelyMeasurable_diffusionIntegrand_path_c12 hv hσmeas S.X_prog j)
    ...
    have hqv : ∀ T' : ℝ, 0 < T' → ∫⁻ ω, ∫⁻ s in Set.Icc (0 : ℝ) T', ∫⁻ e,
        (‖jumpIncrLeft (cutoffFun₂ u (2 * (m : ℝ))) coeffs X.X ω s e‖₊ : ℝ≥0∞) ^ 2
          ∂ν ∂volume ∂P < ⊤ :=
      fun T' hT' => lintegral_sq_jumpIncrement_path_lt_top_c12 hv hK₁ hγmL hγqL hT'
    have hCdv : ∀ᵐ ω ∂P, ω ∈ Brownian.Ito.boundedPathSet X.X T m →
      ...
      filter_upwards [ae_lintegral_compensatorDriftIntegrand_lt_top_c12 hv hK₂ hγmeas hΛm hγmL hγqL
        hT] with ω hω _
      exact integrableOn_window_univ_of_lintegral_lt_top
        ((measurable_compensatorDriftIntegrand_path_c12 hv hγmeas hΛm).comp measurable_prodMk_left)
        hω
    -- the jump side transfers as a sum
    have hjs := ae_jumpSide_cutoffFun₂_eq_c12 N S.ℱ S.isPoisson u hu hγmeas hT hm hTm hXadapt hXleft
    ...
    have hcmpv : ∀ᵐ ω ∂P,
        LevyStochCalc.Poisson.Compensated.stochasticIntegral N S.ℱ S.isPoisson
          ...
          = LevyStochCalc.Poisson.Compensated.stochasticIntegral N S.ℱ S.isPoisson
            ...
            (measurable_jumpIncrement_path_c12 hv hγmeas X.measurable_path)
            (markedProgressivelyMeasurable_jumpIncrement_path_c12 hv hγmeas S.X_prog)
            (fun _ hT' => lintegral_sq_jumpIncrement_path_lt_top_c12 hv hK₁ S.γ_meas S.γ_sq hT')
            T ω :=
      compensatedIntegral_congr_of_countable_ne N S.isPoisson
        ...
        (measurable_jumpIncrement_path_c12 hv hγmeas X.measurable_path) hpv
        (markedProgressivelyMeasurable_jumpIncrement_path_c12 hv hγmeas S.X_prog) hqv
        (fun _ hT' => lintegral_sq_jumpIncrement_path_lt_top_c12 hv hK₁ S.γ_meas S.γ_sq hT') hT
    have hcmpu : ∀ᵐ ω ∂P,
        LevyStochCalc.Poisson.Compensated.stochasticIntegral N S.ℱ S.isPoisson
      ...
      compensatedIntegral_congr_of_countable_ne N S.isPoisson
        ...
        (measurable_jumpIncrement_path_c12 hu hγmeas X.measurable_path) hpu
        (markedProgressivelyMeasurable_jumpIncrement_path_c12 hu hγmeas S.X_prog) hqu
        h_jumpInt_sq hT
...
theorem itoLevyFormula_jumpResidual_of_sdeData_general_c12 (x₀ : Fin n → ℝ)
    (X : JumpDiffusion W N coeffs x₀) (S : LevyStochCalc.Ito.BigJump.SdeData X)
    [S.ℱ.IsRightContinuous]
    (hℱ0 : ∀ t : ℝ, t ≤ 0 → S.ℱ 0 ≤ S.ℱ t)
    (hnull : ∀ s : Set Ω, MeasurableSet s → P s = 0 → MeasurableSet[S.ℱ 0] s)
  ...
  have hmain := itoLevyFormula_jumpResidual_of_sdeData_general_of_leftLim_c12 W N coeffs x₀ XY SY
  ...
  have hBro := multidimIntegral_congr_ae W S.ℱ S.isBrownian
    ...
    (fun s ω => diffusionIntegrand u coeffs.σ s (X.X s ω))
    (fun j => measurable_diffusionIntegrand_path_c12 hu hσmeas hYm j)
    (fun j => progressivelyMeasurable_diffusionIntegrand_path_c12 hu hσmeas hYa j)
    hσY
    (fun j => measurable_diffusionIntegrand_path_c12 hu hσmeas X.measurable_path j)
    (fun j => progressivelyMeasurable_diffusionIntegrand_path_c12 hu hσmeas S.X_prog j)
    h_sigmaGrad_sq
  ...
  have hCmp := compensatedIntegral_congr_ae N S.ℱ S.isPoisson
    ...
    (fun ω' s e => u s (X.X s ω' + coeffs.γ s (X.X s ω') e) - u s (X.X s ω'))
    (measurable_jumpIncrement_path_c12 hu hγmeas hYm)
    (markedProgressivelyMeasurable_jumpIncrement_path_c12 hu hγmeas hYa)
    hγY
    (measurable_jumpIncrement_path_c12 hu hγmeas X.measurable_path)
    (markedProgressivelyMeasurable_jumpIncrement_path_c12 hu hγmeas S.X_prog)
    h_jumpInt_sq
...
theorem itoLevyFormula_general_c12 (x₀ : Fin n → ℝ)
    (X : JumpDiffusion W N coeffs x₀) (S : LevyStochCalc.Ito.BigJump.SdeData X)
    [S.ℱ.IsRightContinuous]
    (hℱ0 : ∀ t : ℝ, t ≤ 0 → S.ℱ 0 ≤ S.ℱ t)
    (hnull : ∀ s : Set Ω, MeasurableSet s → P s = 0 → MeasurableSet[S.ℱ 0] s)
    ...
    ∀ᵐ ω ∂P,
      u T (X.X T ω) - u 0 (X.X 0 ω)
        = (∫ s in Set.Icc (0 : ℝ) T, driftIntegrand u coeffs s (X.X s ω))
          ...
          + ∫ s in Set.Icc (0 : ℝ) T, ∫ e,
              compensatorDriftIntegrand u coeffs.γ s (X.X s ω) e ∂ν := by
  filter_upwards [itoLevyFormula_jumpResidual_of_sdeData_general_c12 W N coeffs x₀ X S hℱ0 hnull
    hμmeas hσmeas hγmeas hμq u hu T hT h_sigmaGrad_sq h_jumpInt_sq h_compDrift_int] with ω hω
  linarith

/-- The Itô–Lévy formula for a jump diffusion and a state function that is `C²` jointly in time
and state, as the case of a `C^{1,2}` state function at a jointly `C²` one. -/
theorem itoLevyFormula_general_of_c12 (x₀ : Fin n → ℝ)
    (X : JumpDiffusion W N coeffs x₀) (S : LevyStochCalc.Ito.BigJump.SdeData X)
    [S.ℱ.IsRightContinuous]
    (hℱ0 : ∀ t : ℝ, t ≤ 0 → S.ℱ 0 ≤ S.ℱ t)
    (hnull : ∀ s : Set Ω, MeasurableSet s → P s = 0 → MeasurableSet[S.ℱ 0] s)
    ...
    ∀ᵐ ω ∂P,
      u T (X.X T ω) - u 0 (X.X 0 ω)
        = (∫ s in Set.Icc (0 : ℝ) T, driftIntegrand u coeffs s (X.X s ω))
          ...
          + ∫ s in Set.Icc (0 : ℝ) T, ∫ e,
              compensatorDriftIntegrand u coeffs.γ s (X.X s ω) e ∂ν :=
  itoLevyFormula_general_c12 W N coeffs x₀ X S hℱ0 hnull hμmeas hσmeas hγmeas hμq u
    (IsC12.of_contDiff hu) T hT h_sigmaGrad_sq h_jumpInt_sq h_compDrift_int
