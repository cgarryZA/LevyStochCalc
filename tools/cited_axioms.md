...
the Itô–Lévy integral of progressively measurable marked integrands. Not yet resolved: the
SDE/BSDEJ layer still takes its Brownian integrands over `W.naturalFiltration` and its
Poisson integrands over `naturalFiltration N` — two filtrations, not the common one of
`(W, N)` (X2-3). Until then: no SDE with coupled random coefficients, no BSDEJ and no PRP is
available.
