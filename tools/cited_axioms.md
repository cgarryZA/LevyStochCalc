...
* **Integrand-class caveat (2026-09-06)**: the `h_progMeas` hypothesis of this theorem admits only integrands that are a.s. constant in `ω` for each time (see "Integrand-class audit" below); the theorem is proved, but for Wiener-type integrals of deterministic integrands. The Brownian integrals were rebuilt over genuine progressive measurability in X2-1; the compensated-Poisson integral is X2-2 (`Plan.md`).
...
* **Scope (recorded 2026-09-06, not yet fixed — work package X2).** The progressive-measurability hypotheses `h_sigmaGrad_progMeas` / `h_jumpInt_progMeas` are relative to the natural filtration of a *single* driver (`naturalFiltration (W.W j)` per Brownian coordinate, `naturalFiltration N` alone), because the library's `L²` integrals (`Brownian/MultidimIto.lean`, `Poisson/Compensated.lean`) are built on that class. For the same reason `JumpDiffusion.is_solution` (`Ito/Setting.lean`) is inhabitable only when `σ(s, X_s) i j` is adapted to `ℱ^{W^j}` alone and `γ(s, X_s, e)` to `ℱ^N` alone — i.e. for uncoupled coefficients. The cited theorem is stated for integrands adapted to a common filtration to which `W` is a Brownian motion and `N` a Poisson random measure (the joint filtration of `(W, N)`); until the integrals are generalized to that filtration (X2), #16 is a faithful statement only on that restricted scope, and the same restriction affects #13a (whose representing integrands are asked to be single-driver adapted while the martingale is a `jointFiltration W N` martingale), #9 and #10. **X2-1 (2026-09-06)**: the Brownian integrands `h_sigmaGrad_progMeas` are now `ProgressivelyMeasurable W.naturalFiltration`, the joint natural filtration of all coordinates of `W`; the Poisson integrand `h_jumpInt_progMeas` is still over `naturalFiltration N` alone, so the scope restriction stands until X2-2/X2-3.
...
* **Replacement plan**: after X2, derive as a theorem: the continuous part through an Itô formula for the library's Brownian integral (candidate: `formal-mathfin`'s `ito_formula_td_process`, one-dimensional, through a Brownian bridge), the jump part in-house on `Compensated.stochasticIntegral` via `itoIsometry_diff_compensated`.
...
(`Brownian/MultidimFiltered.lean`). Not yet resolved: the compensated-Poisson chain (#6, #18)
still carries the old hypothesis (X2-2), and the SDE/BSDEJ layer still takes its Poisson
integrands over `naturalFiltration N` alone, so no common filtration of `(W, N)` is used yet
(X2-3). Until then: no SDE with coupled random coefficients, no BSDEJ and no PRP is available.
