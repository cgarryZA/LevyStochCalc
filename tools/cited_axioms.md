...
* **Carried as a hypothesis (2026-09-16)**: `LevyStochCalc.BSDEJ.PathRegularity.CellRegularity` (`BSDEJ/CellRegularity.lean`) states the rate on uniform grids — `∑ᵢ E∫_cell ‖Z − Z̄ᵢ‖² ≤ C T/N` with `Z̄ᵢ = condCellAverage_Z`, the `𝔸_{tᵢ}`-conditional cell average, and the mark analogue on a finite family — as an explicit named predicate with no proof attached; it is a Level-A input of the dissertation's a-posteriori chain, not a theorem of this library, for the reasons in the replacement plan above. `examples/NonvacuityBSDEJCellRegularity.lean` inhabits it (`C = e²`) for the `f = y`, `ξ = W₁` equation only, where `Z` is deterministic so the conditional and pathwise averages coincide and the `U` half is `0`; the witness exercises the statement, not the stochastic content of the cited theorem.
...
* **`C^{1,2}` form (2026-09-16)**: `itoLevyFormula_general` asks the state function to be `C²` jointly in time and state; the literature (Applebaum Thm 4.4.7, Protter II.7) asks only `C^{1,2}`. `Ito/ItoLevyFormulaGeneralC12.lean` states `itoLevyFormula_general_c12` for `IsC12 u` (`Ito/C12.lean`: continuous, `C¹` in time with jointly continuous time derivative, `C²` in the state with jointly continuous gradient and Hessian — a strictly larger class, witnessed by `(t, x) ↦ t|t| x₀²`), with the statement otherwise unchanged; the pinned theorem is recovered from it as `itoLevyFormula_general_of_c12` (the same type, by `rfl`). The proof mollifies jointly in time and state at the bounded-derivative level (`Ito/C12MollifyJoint.lean`, `Ito/ItoLevyBoundedDerivsC12.lean`) and threads the class through the cut-off and admissibility helpers (`Ito/CutoffC12.lean`, `Ito/JumpFormulaContinuityC12.lean`, `Ito/ItoLevyBoundedDerivsSolutionIntegrandsC12.lean`, `Ito/ItoLevyBoundedDerivsSolutionMainC12.lean`).
