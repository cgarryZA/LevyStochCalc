...
- **`sorry` status (2026-09-11)**: `tools/sorry_baseline.txt` holds exactly one declaration,
  ...
  The papers' quadratic Itô formula in expectation form — `𝔼[X_T²]` for an Itô–Lévy process
  over a Lévy driver under the `L²` hypotheses alone — is `Ito/SecondMoment.lean`
  (2026-09-11, D-Itô-E); the pathwise formula with local integrals stays open (Epic D).
